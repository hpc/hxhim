...
void test_req_and_wait(struct mdhim *md, MPI_Request *req) {
	...
	while (!done) {
		//TODO pthread_mutex_lock(md->mdhim_comm_lock);
		ret = MPI_Test(req, &flag, &status);
		//Unlock the mdhim_comm_lock
		//TODO pthread_mutex_unlock(md->mdhim_comm_lock);
...
int send_rangesrv_work(struct mdhim *md, int dest, void *message) {
	...
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: Packing message "
                     "failed before sending.", md->p->uid);
		return MDHIM_ERROR;
	}
...
int receive_rangesrv_work(struct mdhim *md, int *src, void **message) {
	...
	if ( return_code != MPI_SUCCESS ) {
             	mlog(MDHIM_SERVER_CRIT, "MDHIM Rank: %d - Error: %d "
                     "receive size message failed.", md->p->uid, return_code);
		free(req);
		return MDHIM_ERROR;
	}
	...
	if (return_code == MPI_ERR_IN_STATUS) {
		mlog(MDHIM_SERVER_CRIT, "MDHIM Rank: %d - Received an error status: %d "
                     " while receiving work message size", md->p->uid, status.MPI_ERROR);
	}
	...
	if ( return_code != MPI_SUCCESS ) {
             	mlog(MDHIM_SERVER_CRIT, "MDHIM Rank: %d - Error: %d "
                     "receive message failed.", md->p->uid, return_code);
		free(recvbuf);
		free(req);
		return MDHIM_ERROR;
	}
	...
	*message = NULL;
	...
	//TODO bm = (mdhim_basem_t*)malloc(sizeof(struct mdhim_basem_t));
	...
	*sendbuf = NULL;
	...
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to pack "
                     "the message while sending.", md->p->uid);
		ret = MDHIM_ERROR;
	}
...
int receive_client_response(struct mdhim *md, int src, void **message) {
	...
	*message = NULL;
	...
	//Unpack buffer to get the message type
	//TODO return_code = MPI_Unpack(recvbuf, msg_size, &mesg_idx, bm,
	//			 sizeof(struct mdhim_basem_t), MPI_CHAR,
	//			 md->mdhim_comm);
	mtype = bm->mtype;
	msg_size = bm->size;
	...
	if (return_code != MDHIM_SUCCESS) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to unpack "
                     "the message while receiving from client.", md->p->uid);
		return MDHIM_ERROR;
	}
...
int pack_put_message(struct mdhim *md, struct mdhim_putm_t *pm, void **sendbuf, int *sendsize) {
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: put message too large."
                     " Put is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
	...
	pm->basem.size = mesg_size;
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack put message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_bput_message(struct mdhim *md, struct mdhim_bputm_t *bpm, void **sendbuf, int *sendsize) {
	int return_code = MPI_SUCCESS; // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: bulk put message too large."
                     " Bput is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
	...
	*sendsize = mesg_size;
	bpm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack bulk put message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_put_message(struct mdhim *md, void *message, int mesg_size,  void **putm) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack put message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_bput_message(struct mdhim *md, void *message, int mesg_size, void **bput) {
	...
	int num_records;
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack bput message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_get_message(struct mdhim *md, struct mdhim_getm_t *gm, void **sendbuf, int *sendsize) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: get message too large."
                     " Get is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
        mesg_size = m_size;
        *sendsize = mesg_size;
	gm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack get message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_bget_message(struct mdhim *md, struct mdhim_bgetm_t *bgm, void **sendbuf, int *sendsize) {
	int return_code = MPI_SUCCESS; // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: bulk get message too large."
                     " Bget is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
        mesg_size = m_size;  // Safe size to use in MPI_pack     
	*sendsize = mesg_size;
	bgm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack bulk get message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_get_message(struct mdhim *md, void *message, int mesg_size, void **getm) {
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack get message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_bget_message(struct mdhim *md, void *message, int mesg_size, void **bgetm) {
	...
	int num_records;
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack bget message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_bgetrm_message(struct mdhim *md, struct mdhim_bgetrm_t *bgrm, void **sendbuf, int *sendsize) {
	int return_code = MPI_SUCCESS; // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: bulk get return message too large."
		     " Bget return message is over Maximum size allowed of %d.", md->p->uid,
		     MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
        mesg_size = m_size;  // Safe size to use in MPI_pack     
	*sendsize = mesg_size;
	bgrm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack bulk get return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
//						&mesg_idx, md->mdhim_comm);
	...
	if ( return_code != MPI_SUCCESS ) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to pack "
                     "the bulk get return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_bgetrm_message(struct mdhim *md, void *message, int mesg_size, void **bgetrm) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack bget return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
//				 MPI_CHAR, md->mdhim_comm);
	...
	// Allocate memory for key pointers, to be populated later.
		...
		mlog(MDHIM_SERVER_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
		     "memory to unpack bgetrm message.", md->p->uid);
		return MDHIM_ERROR; 
        }	
...
//						  MPI_CHAR, md->mdhim_comm);
	...
	if ( return_code != MPI_SUCCESS ) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to unpack "
                     "the bget return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_base_message(struct mdhim *md, struct mdhim_basem_t *cm, void **sendbuf, int *sendsize) {
	...
	cm->size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack del message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_del_message(struct mdhim *md, struct mdhim_delm_t *dm, void **sendbuf, int *sendsize) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: del message too large."
                     " Del is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
        mesg_size = m_size;
        *sendsize = mesg_size;
...
int pack_bdel_message(struct mdhim *md, struct mdhim_bdelm_t *bdm, void **sendbuf,
	...
	int return_code = MPI_SUCCESS; // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: bulk del message too large."
                     " Bdel is over Maximum size allowed of %d.", md->p->uid, MDHIM_MAX_MSG_SIZE);
		return MDHIM_ERROR; 
        }
        mesg_size = m_size;  // Safe size to use in MPI_pack     
	*sendsize = mesg_size;
	bdm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack bulk del message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_del_message(struct mdhim *md, void *message, int mesg_size, void **delm) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack del message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_bdel_message(struct mdhim *md, void *message, int mesg_size, void **bdelm) {
	...
	int num_records;
		...
		mlog(MDHIM_SERVER_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
		     "memory to unpack bdel message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int pack_return_message(struct mdhim *md, struct mdhim_rm_t *rm, void **sendbuf, int *sendsize) {
	...
	*sendsize = mesg_size;
	rm->basem.size = mesg_size;
        ...
        if ((*sendbuf = malloc(mesg_size * sizeof(char))) == NULL) {
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to pack return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
...
int unpack_return_message(struct mdhim *md, void *message, void **retm) {
	...
	int return_code = MPI_SUCCESS;  // MPI_SUCCESS = 0
		...
		mlog(MDHIM_CLIENT_CRIT, "MDHIM Rank: %d - Error: unable to allocate "
                     "memory to unpack return message.", md->p->uid);
		return MDHIM_ERROR; 
        }
