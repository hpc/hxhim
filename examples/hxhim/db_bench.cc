...
class Benchmark {
 private:
  ...
  void PrintHeader() {
    const int kKeySize = (16 + sizeof(std::size_t)) * 2 ;
    PrintEnvironment();
      ...
      if (fresh_db) {
        if (FLAGS_use_existing_db) {
          ...
          while ((dirp = readdir(dir))) {
              if (strncmp(dirp->d_name, "mdhimTstDB--", 12) == 0) {
                  DIR *testdir = opendir(dirp->d_name);
                  struct dirent *testdirp = NULL;
                  while ((testdirp = readdir(testdir))) {
                      std::string name(testdirp->d_name);
                      if ((name == ".") || (name == "..")) {
                          continue;
                      }
                      g_env->DeleteFile(std::string(dirp->d_name) + "/" + name);
                  }
                  closedir(testdir);

                  g_env->DeleteDir(dirp->d_name);
              }
              else if (strncmp(dirp->d_name, "mdhim_manifest", 14) == 0) {
                  g_env->DeleteFile(dirp->d_name);
              }
          }
          closedir(dir);
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    void **subjects = new void *[entries_per_batch_];
    std::size_t *subject_lens = new std::size_t[entries_per_batch_];
    void **predicates = new void *[entries_per_batch_];
    std::size_t *predicate_lens = new std::size_t[entries_per_batch_];
    void **objects = new void *[entries_per_batch_];
    std::size_t *object_lens = new std::size_t[entries_per_batch_];

    for (int i = 0; i < entries_per_batch_; i++) {
      subjects[i] = ::operator new(17);
      predicates[i] = ::operator new(17);
    ...
    for (int i = 0; i < num_; i += entries_per_batch_) {
      for (int j = 0; j < entries_per_batch_; j++) {
        ...
        memcpy(objects[j], gen.Generate(value_size_).data(), value_size_);
  ...
  //   for (int i = 0; i < entries_per_batch_; i++) {
  //     subjects[i] = ::operator new(100);
  //     predicates[i] = ::operator new(100);
  //   }

  //   for (int i = 0; i < num_; i += entries_per_batch_) {
  //     for (int j = 0; j < entries_per_batch_; j++) {
  //       const int sub = seq?i:(thread->rand.Next() % FLAGS_num);
  //       subject_lens[j] = snprintf((char *) subjects[j], 100, "%016d", sub);

  //       const int pred = seq?j:(thread->rand.Next() % FLAGS_num);
  //       predicate_lens[j] = snprintf((char *) predicates[j], 100, "%016d", pred);

  //       hxhimDelete(hx, subjects[j], subject_lens[j], predicates[j], predicate_lens[j]);
  //       thread->stats.FinishedSingleOp();
  //     }
