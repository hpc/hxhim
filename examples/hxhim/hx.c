...
int main(int argc, char *argv[]) {
    ...
    for(size_t i = 0; i < count; i++) {
        hxhimPut(&hx, subjects[i], subject_lens[i], predicates[i], predicate_lens[i], HXHIM_SPO_BYTE_TYPE, objects[i], object_lens[i]);
    ...
    for(size_t i = 0; i < count; i++) {
        hxhimGet(&hx, subjects[i], subject_lens[i], predicates[i], predicate_lens[i], HXHIM_SPO_BYTE_TYPE);
    }
    ...
    hxhim_spo_type_t *bget_types = malloc(count * sizeof(hxhim_spo_type_t));
    for(size_t i = 0; i < count; i++) {
