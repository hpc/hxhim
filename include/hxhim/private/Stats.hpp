...
struct Global {
    ...
    std::map<enum hxhim_op_t, std::list<Chronostamp> > single_op;

    // how long each bulk operation called by the user took
    std::map<enum hxhim_op_t, std::list<Chronostamp> > bulk_op;
    ...
    std::map<enum hxhim_op_t, std::list <Chronostamp> > transport;
    ...
    std::ostream &print(const std::map<enum hxhim_op_t, std::size_t> &max_ops_per_send,
                        std::ostream &stream,
