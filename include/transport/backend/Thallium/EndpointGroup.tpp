...
Recv_t *Transport::Thallium::EndpointGroup::do_operation(const std::map<int, Send_t *> &messages) {
    ...
    for(REF(messages)::value_type const &message : messages) {
        ...
        const std::string res = rpc->on(*dst_it->second)(std::string((char *) buf, bufsize));
        ...
        if (Unpacker::unpack(&response, (char *) res.c_str(), res.size(), responses, arrays, buffers) != TRANSPORT_SUCCESS) {
            packed->release(buf, bufsize);
            // mlog(THALLIUM_DBG, "Unable to unpack message");
            continue;
        }

        // mlog(THALLIUM_DBG, "Unpacked message of type %s", Message::TypeStr[response->type]);
...
//         // unpack the response
//         Recv_t *response = nullptr;
//         if (Unpacker::unpack(&response, buf, response_size, responses, arrays, buffers) != TRANSPORT_SUCCESS) {
//             packed->release(buf, bufsize);
//             // mlog(THALLIUM_DBG, "Unable to unpack message");
//             continue;
//         }
