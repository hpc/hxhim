...
TEST(hxhim, TypeMismatch) {
    for(std::size_t i = 0; i < HXHIM_PUT_PERMUTATIONS_COUNT; i++) {
        // Tack on more permutations with each loop
        hxhim_put_permutation_t perms = 0;
        for(std::size_t j = 0; j <= i; j++) {
            perms |= HXHIM_PUT_PERMUTATIONS[j];
        }
        ...
        EXPECT_EQ(put_results->Size(), i + 1);
