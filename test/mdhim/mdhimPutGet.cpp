...
TEST(mdhimPutGet, no_secondary) {
    ...
    struct mdhim_brm_t *brm = mdhimPut(&md, (void *)&MDHIM_PUT_GET_PRIMARY_KEY, sizeof(MDHIM_PUT_GET_PRIMARY_KEY),
                                       (void *)&MDHIM_PUT_GET_VALUE, sizeof(MDHIM_PUT_GET_VALUE), NULL, NULL);
    ...
    //Commit changes
    EXPECT_EQ(mdhimCommit(&md, md.primary_index), MDHIM_SUCCESS);

    //Get value back
    {
        struct mdhim_bgetrm_t *bgrm = mdhimGet(&md, md.primary_index, (void *)&MDHIM_PUT_GET_PRIMARY_KEY, sizeof(MDHIM_PUT_GET_PRIMARY_KEY), MDHIM_GET_EQ);
...
TEST(mdhimPutGet, secondary_global) {
    ...
    struct secondary_info *sg_info = mdhimCreateSecondaryInfo(sg_index, (void **) &sgk_ptr, &sgk_len,
...
TEST(mdhimPutGet, secondary_local) {
    ...
    //Create the secondary info struct
    struct secondary_info *sl_info = mdhimCreateSecondaryInfo(sl_index, (void **) &slk_ptr, &slk_len,
                                                              1, SECONDARY_GLOBAL_INFO);
...
TEST(mdhimPutGet, secondary_global_and_local) {
    ...
    //Put the key-value pair
    struct mdhim_brm_t *brm = mdhimPut(&md, (void *)&MDHIM_PUT_GET_PRIMARY_KEY, sizeof(MDHIM_PUT_GET_PRIMARY_KEY),
                                       (void *)&MDHIM_PUT_GET_VALUE, sizeof(MDHIM_PUT_GET_VALUE), sg_info, sl_info);
    ASSERT_NE(brm, nullptr);
    ...
    {
        struct mdhim_bgetrm_t *bgrm = mdhimGet(&md, md.primary_index, (void *)&MDHIM_PUT_GET_PRIMARY_KEY,
                                               sizeof(MDHIM_PUT_GET_PRIMARY_KEY), MDHIM_GET_EQ);
        ASSERT_NE(bgrm, nullptr);
        EXPECT_EQ(bgrm->error, MDHIM_SUCCESS);
