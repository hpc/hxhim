...
TEST(little_endian, c_string) {
...

    {
        ...
        EXPECT_EQ(little_endian::encode(encoded, orig, len), HXHIM_SUCCESS);
...

    {
        ...
        EXPECT_EQ(little_endian::decode(decoded, orig, len), HXHIM_SUCCESS);
...
TEST(little_endian, int) {
    const int orig = rand();

    char encoded[sizeof(orig)] = {};
    EXPECT_EQ(little_endian::encode(encoded, orig, sizeof(orig)), HXHIM_SUCCESS);

    int decoded = 0;
    EXPECT_EQ(little_endian::decode(decoded, encoded, sizeof(orig)), HXHIM_SUCCESS);
    EXPECT_EQ(decoded, orig);
}

TEST(little_endian, float) {
