...
TEST(elen, decode_integers_error) {
    // non-0 single character string
    EXPECT_THROW(elen_decode_int("1",             NULL, '-', '+'), std::invalid_argument);
...
TEST(elen, decode_small_decimals_error) {
    // empty string
    EXPECT_THROW(elen_decode_large_float("",              '-', '+'), std::invalid_argument);

    // non-0 single character string
    EXPECT_THROW(elen_decode_large_float("1",             '-', '+'), std::invalid_argument);

    // need more than 2 character,
    EXPECT_THROW(elen_decode_large_float("-+",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_large_float("+-",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_large_float("0+",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_large_float("+0",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_large_float("-0",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_large_float("0-",            '-', '+'), std::length_error);

    // bad prefix
    EXPECT_THROW(elen_decode_large_float("?9876+",        '-', '+'), std::invalid_argument);
    EXPECT_THROW(elen_decode_large_float("?0123-",        '-', '+'), std::invalid_argument);
...
TEST(elen, decode_large_decimals_error) {
    // empty string
    EXPECT_THROW(elen_decode_large_float("",              '-', '+'), std::invalid_argument);
    ...
    // bad prefix
    EXPECT_THROW(elen_decode_large_float("?898+",         '-', '+'), std::invalid_argument);
    EXPECT_THROW(elen_decode_large_float("?101-",         '-', '+'), std::invalid_argument);
...
TEST(elen, encode_floating_point) {
    ELEN_EQ(elen_encode_float(-0.1e11,    1, '-', '+'),  "---7888+");
    ELEN_EQ(elen_encode_float(-0.1e10,    1, '-', '+'),  "---7898+");
    ELEN_EQ(elen_encode_float(-1.4,       2, '-', '+'),    "--885+");
    ELEN_EQ(elen_encode_float(-1.3,       2, '-', '+'),    "--886+");
    ELEN_EQ(elen_encode_float(-1,         1, '-', '+'),     "--88+");
    ELEN_EQ(elen_encode_float(-0.123,     3, '-', '+'),    "-0876+");
    ELEN_EQ(elen_encode_float(-0.0123,    3, '-', '+'),   "-+1876+");
    ELEN_EQ(elen_encode_float(-0.001233,  4, '-', '+'),  "-+28766+");
    ELEN_EQ(elen_encode_float(-0.00123,   3, '-', '+'),   "-+2876+");
    ELEN_EQ(elen_encode_float(0,          0, '-', '+'),         "0");
    ELEN_EQ(elen_encode_float(+0.00123,   3, '-', '+'),   "+-7123-");
    ELEN_EQ(elen_encode_float(+0.001233,  4, '-', '+'),  "+-71233-");
    ELEN_EQ(elen_encode_float(+0.0123,    3, '-', '+'),   "+-8123-");
    ELEN_EQ(elen_encode_float(+0.123,     3, '-', '+'),    "+0123-");
    ELEN_EQ(elen_encode_float(+1,         1, '-', '+'),     "++11-");
    ELEN_EQ(elen_encode_float(+1.3,       2, '-', '+'),    "++113-");
    ELEN_EQ(elen_encode_float(+1.4,       2, '-', '+'),    "++114-");
    ELEN_EQ(elen_encode_float(+0.1e10,    1, '-', '+'),  "+++2101-");
    ELEN_EQ(elen_encode_float(+0.1e11,    1, '-', '+'),  "+++2111-");
}

// Chapter 6 Floating Pointer Numbers
TEST(elen, decode_floating_point) {
    EXPECT_FLOAT_EQ(elen_decode_float("---7888+", '-', '+'),   -0.1e11);
    EXPECT_FLOAT_EQ(elen_decode_float("---7898+", '-', '+'),   -0.1e10);
    EXPECT_FLOAT_EQ(elen_decode_float("--885+",   '-', '+'),      -1.4);
    EXPECT_FLOAT_EQ(elen_decode_float("--886+",   '-', '+'),      -1.3);
    EXPECT_FLOAT_EQ(elen_decode_float("--88+",    '-', '+'),        -1);
    EXPECT_FLOAT_EQ(elen_decode_float("-0876+",   '-', '+'),    -0.123);
    EXPECT_FLOAT_EQ(elen_decode_float("-+1876+",  '-', '+'),   -0.0123);
    EXPECT_FLOAT_EQ(elen_decode_float("-+28766+", '-', '+'), -0.001233);
    EXPECT_FLOAT_EQ(elen_decode_float("-+2876+",  '-', '+'),  -0.00123);
    EXPECT_FLOAT_EQ(elen_decode_float("0",        '-', '+'),         0);
    EXPECT_FLOAT_EQ(elen_decode_float("+-7123-",  '-', '+'),  +0.00123);
    EXPECT_FLOAT_EQ(elen_decode_float("+-71233-", '-', '+'), +0.001233);
    EXPECT_FLOAT_EQ(elen_decode_float("+-8123-",  '-', '+'),   +0.0123);
    EXPECT_FLOAT_EQ(elen_decode_float("+0123-",   '-', '+'),    +0.123);
    EXPECT_FLOAT_EQ(elen_decode_float("++11-",    '-', '+'),        +1);
    EXPECT_FLOAT_EQ(elen_decode_float("++113-",   '-', '+'),      +1.3);
    EXPECT_FLOAT_EQ(elen_decode_float("++114-",   '-', '+'),      +1.4);
    EXPECT_FLOAT_EQ(elen_decode_float("+++2101-", '-', '+'),   +0.1e10);
    EXPECT_FLOAT_EQ(elen_decode_float("+++2111-", '-', '+'),   +0.1e11);
}

TEST(elen, decode_floating_point_error) {
    // empty string
    EXPECT_THROW(elen_decode_float("",              '-', '+'), std::invalid_argument);

    // non-0 single character string
    EXPECT_THROW(elen_decode_float("1",             '-', '+'), std::invalid_argument);

    // need more than 2 character,
    EXPECT_THROW(elen_decode_float("-+",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_float("+-",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_float("0+",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_float("+0",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_float("-0",            '-', '+'), std::length_error);
    EXPECT_THROW(elen_decode_float("0-",            '-', '+'), std::length_error);

    // bad prefix
    EXPECT_THROW(elen_decode_float("?88+",          '-', '+'), std::invalid_argument);
    EXPECT_THROW(elen_decode_float("?11-",          '-', '+'), std::invalid_argument);
...
TEST(elen, random_floating_point) {
    ...
    for(std::size_t i = 0; i < 10; i++) {
        const float value = dist(gen);
        floats.push_back(value);
        strings.push_back(elen_encode_float(value, ELEN_ENCODE_FLOAT_PRECISION, ELEN_NEG, ELEN_POS));
    }
    ...
    std::vector<char *>::const_iterator s_it = strings.begin();
    while (f_it != floats.end()) {
        EXPECT_NEAR(*f_it, elen_decode_float(*s_it, ELEN_NEG, ELEN_POS), ELEN_ENCODE_FLOAT_PRECISION);
        free(*s_it);
        f_it++;
        s_it++;
    }
