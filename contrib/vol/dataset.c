...
herr_t H5VL_hxhim_dataset_read(void *dset, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
    ...
    fprintf(stderr, "%4d %s   buf: %p\n", __LINE__, __func__, buf);

    int rc = (hxhimGet(&dataset_info->file->under_vol->hx,
                       dataset_info->subject, dataset_info->subject_len,
                       (void *) predicate, predicate_len,
                       HXHIM_OBJECT_TYPE_BYTE) == HXHIM_SUCCESS);
    ...
    // need to flush at some point so that object can be copied into buf
...
herr_t H5VL_hxhim_dataset_write(void *dset, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
    ...
    const int rc = (hxhimPut(&dataset_info->file->under_vol->hx,
                             dataset_info->subject, dataset_info->subject_len,
                             (void *) predicate, predicate_len,
                             HXHIM_OBJECT_TYPE_BYTE, (void *) buf, size * count) == HXHIM_SUCCESS);
