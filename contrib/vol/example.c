...
int main(int argc, char * argv[]) {
    ...
    /* write */
    {
        ...
        {
            hid_t dataspace_id = H5Screate_simple(1, &dims, NULL);
            hid_t dataset_id = H5Dcreate(file_id, "/floats", H5T_NATIVE_FLOAT, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

            float write_floats[COUNT];
            for(hsize_t i = 0; i < dims; i++) {
                write_floats[i] = i;
            }

            /* Write the dataset */
            H5Dwrite(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, write_floats);
        ...
        }
        {
            ...
            /* Write the dataset. */
            H5Dwrite(dataset_id, H5T_C_S1, H5S_ALL, H5S_ALL, H5P_DEFAULT, write_str);
            printf("    write %s\n", write_str);
    ...
    /* read */
    {
        ...
        {
            hid_t dataset_id = H5Dopen(file_id, "/floats", H5P_DEFAULT);

            float read_floats[COUNT];
            H5Dread(dataset_id, H5T_NATIVE_FLOAT, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, read_floats);
            for(hsize_t i = 0; i < COUNT; i++) {
                printf("    read %f\n", read_floats[i]);
            }
        ...
        }
        {
            ...
            char read_str[1024];
            H5Dread(dataset_id, H5T_C_S1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, read_str);
            printf("    read %s\n", read_str);
